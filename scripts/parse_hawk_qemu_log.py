...
RUN_RE = re.compile(
    ...
    r"pk=(?P<pk>[0-9a-f.]+) sm=(?P<sm>[0-9a-f.]+) sm_len=(?P<sm_len>\d+)"
)
AVG_RE = re.compile(
...
def parse_log(text: str, log_path: Path, artifacts: dict | None = None) -> dict:
            ...
            runs.append(
                {
                    ...
                    "sm_prefix": match.group("sm"),
                    "signed_message_bytes": int(match.group("sm_len")),
                }
            )
            continue
    ...
    if not runs:
        raise ValueError("No se encontraron ejecuciones HAWK en el log")
...
def to_markdown(data: dict) -> str:
    lines = [
        "# Resultado automatizado HAWK-512 sobre OP-TEE/QEMU",
        "",
        ...
        "",
        "| Run | NOP (us) | Fresh session NOP (us) | Keygen (us) | Sign (us) | Verify (us) | verify_ok | sm bytes | pk prefix | sm prefix |",
        "| --- | ---: | ---: | ---: | ---: | ---: | --- | ---: | --- | --- |",
    ]
    ...
    for run in data["runs"]:
        lines.append(
            ...
            f"{int(run['verify_ok'])} | {run['signed_message_bytes']} | "
            f"{run['pk_prefix']} | {run['sm_prefix']} |"
        )
    ...
    if data["artifact_sizes"]:
        lines.extend(
            [
                "",
                "## Tamanos de artefactos",
                "",
                "Estos valores son tamanos de artefactos compilados; no son medidas de RAM ni de flash real en hardware.",
                "",
